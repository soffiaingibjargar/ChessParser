                      ...
                      verbose = getOption("verbose"), ...) 
{
  ...
  par(mar = c(margins[1L], 0, 3, margins[2L]))
  if (!symm || scale != "none") 
    x <- t(x)
  if (revC) {
    iy <- nr:1
  ...
  axis(3, 1L:nc, labels = labCol, las = 1, line = -0.5, tick = 0, 
       cex.axis = cexCol)
  if (!is.null(xlab)) 
    mtext(xlab, side = 2, line = margins[1L] - 7.25, padj = 25.5)
...
chessHeatmap = function (x, Rowv = NULL, Colv = if (symm) "Rowv" else NULL, 
                         distfun = dist, hclustfun = hclust, reorderfun = function(d, 
                                                                                   w) reorder(d, w), add.expr, symm = FALSE, revC = identical(Colv, 
                                                                                                                                              "Rowv"), scale = c("row", "column", "none"), na.rm = TRUE, 
                         margins = c(5, 5), ColSideColors, RowSideColors, cexRow = 0.2 + 
                         ...
                         verbose = getOption("verbose"), ...) 
{
  ...
  axis(4, iy, labels = labRow, las = 1, line = -0.5, tick = 0, 
       cex.axis = cexRow)
  if (!is.null(ylab)) 
...
testHeatmap <- function (x, Rowv = NULL, Colv = if (symm) "Rowv" else NULL, 
                      ...
                      RowSideColors, cexRow = 0.2 + 1/log10(nrow(x)), cexCol = 0.2 + 
                        1/log10(ncol(x)), lasRow = 1, lasCol = 1, labRow = NULL, 
                      labCol = NULL, main = NULL, xlab = NULL, ylab = NULL, keep.dendro = FALSE, 
                      verbose = getOption("verbose"), useRaster = if (ncol(x) * 
                                                                        nrow(x) >= 50000) TRUE else FALSE, ...) 
{
  if (!all(is.na(topN))) {
  ...
  if (!missing(RowSideColors)) {
    ...
    if (dim(rsc)[2] == 1) {
      axis(1, 0, RowSideLabs, las = 1, tick = FALSE)
    }
    else {
  ...
  if (!missing(ColSideFun)) {
    ...
    if (dim(csc)[2] == 1) {
      axis(4, 0, ColSideLabs, las = 1, tick = FALSE)
    }
    else {
